...
import (
	...
	"github.com/migelankodra/application-export/internal/store/contracts"
...
func (sf *storeForwardInfo) storeForLaterRetry(
	...
	item := contracts.NewStoredObject(sf.runtime.ServiceKey, payload, pipeline.Id, pipelinePosition, pipeline.Hash, appContext.GetAllValues(), topic)
...
func (sf *storeForwardInfo) retryStoredData(serviceKey string) {
	...
	lc.Infof("%d stored data items found for retrying", len(items))

	if len(items) > 0 {
		itemsToRemove, itemsToUpdate := sf.processRetryItems(items)

		lc.Infof(" %d stored data items will be removed post retry", len(itemsToRemove))
		lc.Infof(" %d stored data items will be update post retry", len(itemsToUpdate))
...
func (sf *storeForwardInfo) processRetryItems(items []contracts.StoredObject) ([]contracts.StoredObject, []contracts.StoredObject) {
	...
	for _, item := range items {
		pipeline := sf.runtime.GetPipelineById("retry-handler")
		/*
			if pipeline == nil {
