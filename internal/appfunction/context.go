...
func (appContext *Context) SetRetryData(payload []byte, topic string) {
	appContext.retryData = payload
	appContext.topic = topic
}

// RetryData returns the context's retryData. This function is not part of the AppFunctionContext interface,
// so it is internal SDK use only
func (appContext *Context) RetryData() ([]byte, string) {
	return appContext.retryData, appContext.topic
}
